...
thin_surface_dataset <- function(sample_data, n_keep = 512L, seed = 1L) {
  if (!inherits(sample_data$dataset, "mvpa_surface_dataset")) {
    return(sample_data)
  }
  set.seed(seed)
  mask <- sample_data$dataset$mask
  active <- which(mask > 0)
  keep <- sample(active, size = min(length(active), n_keep))
  thin_mask <- integer(length(mask))
  thin_mask[keep] <- 1L
  sample_data$dataset$mask <- thin_mask
  sample_data
}
...
test_that("standard mvpa_searchlight and custom cross-validation runs without error", {
  ...
  sample_set <- replicate(3, {
    list(train=sample(1:24), test=sample(25:36))
  }, simplify=FALSE)
...
test_that("standard surface-based mvpa_searchlight runs without error", {
  ...
  dataset <- thin_surface_dataset(
    gen_sample_dataset(D = 0, nobs = 36, data_mode = "surface"),
    n_keep = 512L
  )
  cval <- blocked_cross_validation(dataset$design$block_var)
  model <- load_model("sda_notune")
...
test_that("randomized surface-based mvpa_searchlight runs without error", {
  skip_if_not_extended_tests()
  dataset <- thin_surface_dataset(
    gen_sample_dataset(D = 30, nobs = 24, data_mode = "surface"),
    n_keep = 384L
  )
  cval <- blocked_cross_validation(dataset$design$block_var)
  model <- load_model("sda_notune")
...
test_that("randomized mvpa_searchlight runs without error", {
  dataset <- gen_sample_dataset(c(4,4,4), 36, nlevels=2)
  cval <- blocked_cross_validation(dataset$design$block_var)
  model <- load_model("sda_notune")
  mspec <- mvpa_model(model, dataset$dataset, dataset$design, 
                      model_type="classification", crossval=cval)
  res <- run_searchlight(mspec,radius=2, method="randomized", niter=1)
  expect_true(!is.null(res))
...
test_that("mvpa_searchlight with sda_boot", {
  ...
  dataset <- gen_sample_dataset(c(4,4,4), 36, nlevels=4, blocks=3)
  cval <- blocked_cross_validation(dataset$design$block_var)
  model <- load_model("sda_boot")
  mspec <- mvpa_model(model, dataset$dataset, dataset$design, model_type="classification", crossval=cval)
  res <- run_searchlight(mspec,radius=2, method="standard", niter=2)
  expect_true(!is.null(res))
...
test_that("randomized mvpa_searchlight runs with custom_performance", {
   ...
   dataset <- gen_sample_dataset(c(4,4,3), 36, nlevels=3)
   cval <- blocked_cross_validation(dataset$design$block_var)
   model <- load_model("sda_notune")
...
test_that("randomized mvpa_searchlight and tune_grid runs without error", {
  ...
  dataset <- gen_sample_dataset(c(4,4,4), 36, nlevels=2, blocks=2)
  cval <- blocked_cross_validation(dataset$design$block_var)
...
test_that("randomized mvpa_searchlight works with regression", {
  ...
  res <- run_searchlight(mspec, radius=2, niter=1, method="standard")
  expect_true(!is.null(res))
})

test_that("randomized mvpa_searchlight works with xgboost", {
  skip_if_not_extended_tests()
  dataset <- gen_sample_dataset(c(3,3,3), 30, nlevels=3, blocks=3)
  cval <- blocked_cross_validation(dataset$design$block_var)
  tuneGrid <- expand.grid(max_depth=2, eta=.5, nrounds=5,gamma=0,colsample_bytree=.6, min_child_weight=1, subsample=.5)
  model <- load_model("xgbTree")
  mspec <- mvpa_model(model, dataset$dataset, dataset$design, crossval=cval, tune_grid=tuneGrid)
  res <- run_searchlight(mspec, radius=2, niter=1,method="randomized")
  expect_true(!is.null(res))
...
test_that("mvpa_searchlight works with testset", {
  require("sda")
  dataset <- gen_sample_dataset(c(3,3,3), 36, response_type="categorical", data_mode="image", 
...
test_that("mvpa_searchlight works with split_var", {
  dataset <- gen_sample_dataset(c(4,4,4), 36, blocks=3, split_by=factor(rep(1:3, each=12)))
