...
This document traces the complete pipeline from model predictions to
final performance metrics in rMVPA’s searchlight and regional analyses.
It identifies the required data structures at each stage and compares
hrfdecoder’s current implementation against the standard contracts.
...
3.  Calls
    ...
    dispatcher for each ROI
4.  Returns tibble with columns: `result`, `indices`, `performance`,
    `id`, `error`, `error_message`

**Key insight**:
[`process_roi()`](http://bbuchsbaum.github.io/rMVPA/reference/process_roi-methods.md)
is a generic dispatcher that routes to `internal_crossval()` or
`external_crossval()` based on whether the model has a test set.
...
**train_model** (`R/mvpa_model.R`, called from various model
implementations): - Receives:
`(obj, train_dat, ytrain, sl_info, cv_spec, indices)` - Returns: Fitted
model object
...
**IGNORES `y` parameter** (it’s a dummy TR sequence) - Uses
`obj$design$event_model` and `obj$design$events` for targets - Calls
`hrfdecoder::fit_hrfdecoder()` on TR-level data - Returns:
`hrfdecoder_fit_wrap` object
...
**Key Differences**: - `y_true` comes from `aggregate_events()`, NOT
from `context$ytest` - `context$ytest` contains dummy TR indices (1:T),
which are IGNORED - `test_ind` points to TRs, but downstream only uses
...
**wrap_result()** (`R/mvpa_model.R:3`): - Aggregates probabilities
across folds for each test observation - Calls
...
constructor - Returns: Classification/regression result object
...
**Key Differences**: - Does NOT use `wrap_result()` (which expects
trial-aligned test indices) - Directly concatenates event-level
predictions from all folds - Each fold contributes different events
(blocked by run) - No probability averaging needed (each event appears
once)
...
**Critical**: Only `observed`, `predicted`, and `probs` are used for
performance computation. The `testind` field is optional and only used
for subsetting in some contexts.
...
`get_binary_perf(split_list)` → calls
`performance.binary_classification_result()` -
`get_multiclass_perf(split_list, class_metrics)` → calls
`performance.multiway_classification_result()` -
`get_regression_perf(split_list)` → calls
...
**Default**:
`get_multiclass_perf(design$split_groups, class_metrics=TRUE)` set at
line 143.
...
**Critical**: `y_true` MUST contain the actual target labels for the
observations being predicted. This is used directly by
...
1.  **format_result.hrfdecoder_model** (line 239):
    - Returns all required fields: `class`, `probs`, `y_true`,
      `test_ind`, `fit`, `error`, `error_message`
    - ✅ `y_true` contains **event-level labels** from
      `aggregate_events()`, NOT dummy TRs
    - ✅ Correctly ignores `context$ytest` (which contains dummy TR
      indices)
2.  **merge_results.hrfdecoder_model** (line 346):
    ...
    - Returns tibble with `result`, `indices`, `performance`, `id`,
      `error`, `error_message`
    - ✅ No dependency on trial-level alignment
3.  **classification_result construction**:
    - `y_true`, `predicted`, and `probs` all have event-level
      granularity
...
**The dummy `y_train` exists ONLY for CV fold construction, NOT for
predictions.**
...
- Aggregated event labels (from `aggregate_events()`) flow to
  performance metrics
- `context$ytest` (dummy TR indices) is **intentionally ignored** in
  `format_result`

### No Pipeline Breaks:

❌ **NO** breaks in the data flow ❌ **NO** missing transformations ❌
**NO** structural mismatches ✅ **ALL** contracts satisfied ✅ **FULL**
compatibility with `yardstick` metrics
...
**hrfdecoder treats `context$ytest` as opaque because it contains dummy
values that are meaningless for performance evaluation.** The real
labels come from the event metadata stored in the design object, which
is accessed via `aggregate_events()`.

This is **by design** and **fully supported** by the S3 dispatch system.
The
[`format_result()`](http://bbuchsbaum.github.io/rMVPA/reference/format_result.md)
contract only requires that `y_true` contain meaningful labels—it
doesn’t mandate where those labels come from.
...
[`y_train.hrfdecoder_model()`](http://bbuchsbaum.github.io/rMVPA/reference/y_train.hrfdecoder_model.md)
returns TR-count-length dummy sequence
...
aggregates TR predictions to events

Event labels extracted from `aggregate_events()`, not `context$ytest`

Returns tibble with all required fields (`class`, `probs`, `y_true`, …)

[`merge_results.hrfdecoder_model()`](http://bbuchsbaum.github.io/rMVPA/reference/merge_results.hrfdecoder_model.md)
concatenates event-level outputs
...
[`compute_performance.hrfdecoder_model()`](http://bbuchsbaum.github.io/rMVPA/reference/compute_performance.hrfdecoder_model.md)
uses standard performance function

Compatible with
[`yardstick::accuracy_vec()`](https://yardstick.tidymodels.org/reference/accuracy.html)
and
[`yardstick::roc_auc_vec()`](https://yardstick.tidymodels.org/reference/roc_auc.html)

No structural mismatches in result objects

All contracts satisfied at every pipeline stage

### Potential Edge Cases:

⚠️ **Events with zero probabilities**: Already handled with warning
(line 319-323) ⚠️ **Events outside TR range**: Validated in design
constructor (line 89-95) ⚠️ **Single run**: Warning issued during model
construction (line 118-120)
...
The hrfdecoder integration is **fully compliant** with rMVPA’s
performance metrics pipeline. The key architectural pattern—using dummy
`y_train` for fold construction while pulling real labels from design
metadata—is elegant and requires **no modifications to the core
framework**.

### Data Flow Correctness:

1.  ✅ Dummy `y_train` used only for CV fold creation
2.  ✅ TR-level predictions aggregate to event-level within
    [`format_result()`](http://bbuchsbaum.github.io/rMVPA/reference/format_result.md)
3.  ✅ Event labels flow from `aggregate_events()` → `y_true` →
...
- **S3 extensibility**: No changes to `internal_crossval()`,
  [`merge_results()`](http://bbuchsbaum.github.io/rMVPA/reference/merge_results.md),
  or
  [`performance()`](http://bbuchsbaum.github.io/rMVPA/reference/performance-methods.md)
- **Clean separation**: Event metadata in design, not polluting CV
  machinery
- **Type safety**:
  [`classification_result()`](http://bbuchsbaum.github.io/rMVPA/reference/classification_result.md)
  enforces contracts at construction
- **Reusability**: Pattern generalizes to any continuous decoder

### No Breaks Identified:

After tracing the complete pipeline from
...
→
[`performance()`](http://bbuchsbaum.github.io/rMVPA/reference/performance-methods.md)
→ `yardstick` metrics, **no data flow breaks were found**.

The hrfdecoder implementation follows the established contracts at every
stage and produces performance metrics that are structurally identical
to standard MVPA models.
...
- `/Users/bbuchsbaum/code/rMVPA/R/mvpa_model.R` - Standard model
  reference
...
- `/Users/bbuchsbaum/code/rMVPA/R/hrfdecoder_model.R` - hrfdecoder
  adapter
- `/Users/bbuchsbaum/code/rMVPA/R/hrfdecoder_design.R` - Design
  specialization
- `/Users/bbuchsbaum/code/rMVPA/R/allgeneric.R` - Generic definitions

------------------------------------------------------------------------

**Generated**: 2025-11-10 **Package**: rMVPA **Analysis**: Complete
performance metrics pipeline trace with hrfdecoder compliance
verification
