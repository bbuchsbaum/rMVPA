...
These notes capture the first installment of a proposal for a new
**Representational Network Analysis (ReNA)** module in `rMVPA`. The goal
is to extend the package beyond per‑ROI/searchlight analyses of *what*
is represented, to analyses of **how representational geometries flow
through networks of regions and models**.

ReNA treats ROIs and searchlights as **nodes in a representational
network**, linked by:

- **Representational connectivity**: who shares what representational
  geometry with whom?
- **Representational mediation**: does this node mediate the effect of a
  model or another region on memory/behavior?
- **Representational mapping (optional)**: how does this node transform
  the representational code it receives?
...
- designed to expose **crisp, per‑ROI metrics** that can be turned into
  maps via
...
Conceptually, this is “hyperalignment / information connectivity /
mediation analysis, but all in RDM space”, integrated into the rMVPA
ecosystem.
...
- A first‑class framework to ask:
  ...
  - **Which regions carry, transform, or mediate representational
    content from A → B → behavior?**
...
1.  **Seed‑based Representational Connectivity (ReNA‑RC)**  
    Basic but powerful: “where in the brain instantiates a
    representational geometry similar to a seed, above confounds?”
2.  **Representational Mediation (ReNA‑RM)**  
    Mediation in RDM space: “does this ROI’s geometry mediate the
    influence of a model/region on another region or behavior?”
3.  **(Optional) Representational Mapping Layer (ReNA‑MAP)**  
    Uses RRR/REMAP‑style mappings to characterize how regions transform
...
and share design helpers and confound handling patterns with
...
- A **target searchlight/ROI**, for which rMVPA already computes a
  neural RDM `D^{ROI}`.

For each target ROI, we ask:

> “Does this ROI instantiate a representational geometry similar to the
> seed’s, above and beyond cheap confounds (run, time, category, etc.)?”
...
- `conn_seed_partial` / `sp_seed`: “where in the brain shares the seed
  geometry above confounds?”

This is essentially “information/representational connectivity”,
implemented in:
...
is an RSA‑design‑lite helper for connectivity:
...
- Confounds are supplied as RDMs and converted to matrices; lower
  triangles are used downstream.

### 2.3 API Sketch – Model Spec

[`repnet_model()`](http://bbuchsbaum.github.io/rMVPA/reference/repnet_model.md)
constructs a model specification compatible with
[`run_searchlight()`](http://bbuchsbaum.github.io/rMVPA/reference/run_searchlight.md)
/
[`run_regional()`](http://bbuchsbaum.github.io/rMVPA/reference/run_regional-methods.md):
...
- `distfun` follows existing RSA conventions (`cordist("pearson")`,
  etc.).
- `simfun` specifies how to compute simple similarity metrics if
  desired.
- `compute_performance = TRUE` ensures per‑ROI metrics (connectivity
...
2.  **Align item order**
    ...
    - Ensure `D_roi` and `seed_rdm` are aligned to the same item order
      specified by `key`.
...
5.  **Extract connectivity metrics**
    ...
    - `sp_seed` (optional): computed via `run_lm_semipartial()` or
      similar helper.
...
produces whole‑brain/ROI maps of representational connectivity to the
seed geometry, with confound control.
...
- “Does region M mediate the representational influence of a model or
  region A on retrieval/behavior?”
- “Does a candidate region M carry transformations from a predictor
  geometry to an outcome geometry?”

Setup:

- **Predictor RDM** `D^X` (e.g., model semantics, low‑level vision,
  encoding region A).
...
4.  Indirect effect: `a × b` (optionally with Sobel or bootstrap
    approximations)

Per ROI, we obtain:

- `a_roi`, `b_roi`, `cprime_roi`, `indirect_roi`, plus simple
  significance approximations if desired.
...
ReNA‑RM then identifies **where in cortex the geometry both follows
semantics and propagates that semantics to hippocampal retrieval
geometry**.

### 3.2 API Sketch – Design Helper

[`repmed_design()`](http://bbuchsbaum.github.io/rMVPA/reference/repmed_design.md)
defines the predictor and outcome RDMs plus confounds:
...
wraps the design into a model spec:
...
4.  **Handle confounds**
    ...
    - Option B: include confounds directly as predictors in the
      regression formulas.
...
This provides a causal‑ish, network‑level story about representations in
a standard, RDM‑native searchlight framework.
...
A third variant would integrate REMAP / reduced‑rank regression (RRR) to
characterize **transformations** between seed and target patterns,
beyond simple connectivity/mediation:

- Instead of comparing RDMs, learn a mapping between seed patterns and
  target patterns using RRR (as in REMAP).
- Summarize:
  - mapping rank,
  - explained variance,
...
This is “90% written already” via existing REMAP code; the main work is
exposing it as a **representational connectivity/mapping primitive**.
...
- **Design helpers**
  - [`repnet_design()`](http://bbuchsbaum.github.io/rMVPA/reference/repnet_design.md)
    – seed + confounds.
  - [`repmed_design()`](http://bbuchsbaum.github.io/rMVPA/reference/repmed_design.md)
    – predictor, outcome, confounds.
  - (Optional)
    [`repmap_design()`](http://bbuchsbaum.github.io/rMVPA/reference/repmap_design.md)
    – mapping‑oriented helper if needed.
- **Model specs**
  - [`repnet_model()`](http://bbuchsbaum.github.io/rMVPA/reference/repnet_model.md)
    → representational connectivity searchlights/ROIs.
  - [`repmed_model()`](http://bbuchsbaum.github.io/rMVPA/reference/repmed_model.md)
    → representational mediation searchlights/ROIs.
  - (Optional)
    [`repmap_model()`](http://bbuchsbaum.github.io/rMVPA/reference/repmap_model.md)
    → mapping searchlights/ROIs.
- **Standard runners**
  - `run_regional(repnet_model, ...)`
...
- **Block/lag/behavior confounds**: handled via RDMs/residualization as
  above.
...
What it lacks is a **coherent, RDM‑native representational network
framework** that:

- treats connectivity, mediation, and mapping as **first‑class
  per‑searchlight analyses**,
- plugs into a **clean MVPA/RSA ecosystem**,
- is built on **honest cross‑validation and confound control**.

ReNA would provide:

> “Not just what each voxel represents, but who talks to whom, how, and
> on which representational axes, in a principled and inspectable way.”
...
as a first installment of the Representational Network module, wired
into existing searchlight/regional infrastructure.
...
- avoid GPUs/transformers while doing things Python users typically
  hand‑roll.

Below is a structured summary of the provided code, with function
...
**Goal:** per ROI/searchlight, quantify how similar its representational
geometry is to a **seed RDM**, optionally controlling for **confound
...
- Uses `design$train_design` and `parse_variable(key_var, d)` to extract
  item identities; coerces to a factor `key_fac`.

- Coerces `seed_rdm` to a square matrix `S`:

  - If `"dist"`: uses `Labels` attribute if present; otherwise falls
    back to levels of `key_fac` and the `"Size"` attribute.
  - Ensures `rownames(S)` and `colnames(S)` are set to item labels.

- Handles `confound_rdms`:

  - Must be a **named list**.
  - Each element can be `"dist"` or matrix; converted to matrices with
    row/colnames (using labels, or `rownames(S)`, or `key_fac` levels as
    defaults).

- Returns a list:

  - `key` – factor of item IDs (length = number of training
    observations),
...
- Asserts `dataset` inherits `"mvpa_dataset"` and `design` inherits
  `"mvpa_design"`.
...
Note: **Per‑ROI performance metrics are computed inside
`process_roi.repnet_model()`**, not by generic `compute_performance`.
...
1.  **Extract ROI data**
    ...
    - If `n_obs < 2` or `n_vox < 1`: return a single‑row tibble with
      `error = TRUE` and NULL performance.

2.  **Check key length**

    - `key <- mod_spec$key`; must have `length(key) == n_obs`, else
      error tibble.

3.  **Item‑level prototypes**

    - `E_full <- group_means(X, margin = 1, group = key)` → `K_roi × V`
      matrix.
...
4.  **Align with seed RDM**
    ...
    - `common_items <- intersect(items_roi, items_seed)`; require at
      least 3 items.
...
5.  **Drop constant voxels**
    ...
    - `sd_E <- apply(E, 2, sd, na.rm = TRUE)`; keep voxels with
      `sd > 0`.
...
7.  **Raw connectivity**
    ...
    - `conn_raw <- cor(d_roi, d_seed, method = mod_spec$simfun, use = "complete.obs")`
      (with
...
8.  **Build regression predictors (seed + confounds)**
    ...
    - For each named confound RDM:
      - Align rows/cols to `common_items` if possible (matching
        rownames).
...
9.  **Multiple regression + semipartials**
    ...
    - Fit `fit <- lm(d_roi ~ ., data = df)` with `try(...)`:
      - Extract non‑intercept coefficient estimates and name them
        `beta_<predictor>`.
    - If a function `run_lm_semipartial()` exists:
      - Build `tmp_obj <- list(design = list(model_mat = mm))`.
      - Call `run_lm_semipartial(dvec = d_roi, obj = tmp_obj)`; if
        numeric, prefix names with `"sp_"`.
...
11. **Return tibble row**
    ...
    - `id = rnum`,
    - `error = FALSE`, `warning = FALSE` (or TRUE with messages in
      failure cases).
...
**Goal:** for each ROI/searchlight, test whether its RDM **mediates**
between a predictor RDM `X` and an outcome RDM `Y` in RDM space,
optionally with confound RDMs.
...
- Uses an internal helper `as_mat()` to turn `"dist"` or matrices into
  square matrices with row/colnames:

  - `"dist"`: uses `Labels` if present; else uses
    `default_items[seq_len(Size)]`.
  - Matrices with missing row/colnames get both set from
    `default_items`.
...
- `confound_rdms`:
  ...
  - optional named list of RDMs, each passed through `as_mat()` with
    `default_items = items`.
...
Again, ROI‑level mediation metrics are supplied by
`process_roi.repmed_model()`.
...
7.  **Confound vectors (optional)**
    ...
    - If `confound_rdms` present:
      - For each `Cr`:
        ...
        - vectorize lower tri.
      - Bind into `conf_mat` with column names = names of
        `confound_rdms`.
...
**Goal:** for each ROI/searchlight, fit a **linear reduced‑rank
mapping** from **seed features** (e.g., model embeddings or patterns
from another region) to ROI patterns. Summarize each ROI by mapping
rank, fit (R²), and mapping norm.
...
- Handles `rank` argument:
  ...
  - If `ridge_lambda` not `NULL`:
    - Uses
      ...
      with `nrank` set to either `max_rank` (for `"auto"`) or fixed
      integer rank.
  - Else if `rank == "auto"`:
    - Uses
      [`rrpack::cv.rrr()`](https://rdrr.io/pkg/rrpack/man/cv.rrr.html)
      with:
      - `nfold = min(5, max(2, floor(n/3)))`,
      - `maxrank = max_rank`.
  - Else:
    - Uses
      [`rrpack::rrr.fit()`](https://rdrr.io/pkg/rrpack/man/rrr.fit.html)
      with `nrank = rank`.

- Extracts mapping matrix and singular values:

  - `C_hat` from `fit$coef` if present, otherwise `rrpack::coef(fit)`;
    wrapped in `tryCatch` with zero fallback.
  - `singvals` from `fit$Ad` (`numeric(0)` on error).
  - `r_used` from `fit$rank` if available, else `length(singvals)`, else
    `NA`.
...
- All three models (`repnet_model`, `repmed_model`, `repmap_model`) are
  **S3 model specs** with `process_roi.*` methods.
- They integrate with existing infrastructure:
  - [`mvpa_iterate()`](http://bbuchsbaum.github.io/rMVPA/reference/mvpa_iterate.md)
    ...
    /
    [`run_regional()`](http://bbuchsbaum.github.io/rMVPA/reference/run_regional-methods.md)
    can dispatch on their classes similarly to `rsa_model`,
    `vector_rsa_model`, `contrast_rsa_model`, etc.
- They do **not** rely on generic cross‑validation helpers:
  - Per‑ROI metrics are computed directly inside `process_roi.*` and
    returned in the `performance` list‑column.
- Combiners can:
  - Reuse
    [`combine_rsa_standard()`](http://bbuchsbaum.github.io/rMVPA/reference/combine_rsa_standard.md)
...
- **`repmed_model` – ReNA‑RM mediation maps**  
  “Where does this region’s geometry mediate model →
  retrieval/behavior?”
- **`repmap_model` – ReNA‑Map mapping maps**  
  “Where does this region implement a strong low‑rank mapping from seed
  features to neural patterns?”

All within the existing rMVPA framework, in R, and in an RDM‑native,
linear‑model aesthetic that’s deliberately **slicker and more coherent**
than what’s typically available in Python toolboxes today.
