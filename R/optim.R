...
#' @param Pred the matrix of predictors
...
#' greedyOptAUC(x, y)
greedyTwoClassOptim <- function(X, Y, iter = 100L){ 
...
greedyMultiClassOptim <- function(PredList, Y, iter = 100L, metric="AUC", oneByOne=FALSE) { 
  ...
  if (oneByOne) {
    ...
    N           <- length(PredList)
    weights     <- rep(0L, N)
    pred        <- matrix(0, length(Y), ncol(PredList[[i]]))
    sum.weights <- 0L
...
#' y <- sample(c('Y', 'N'), 5, replace=TRUE)
#' greedOptAUC(x, y)
greedMultiClassOptAUC <- function(PredList, Y, iter = 100L, bagIter=20, bagFrac=.5){ 
  if(is.character(Y)){
    Y <- factor(Y)
  }
  ...
  sum.weights <- 0L
  stopper     <- max(unlist(lapply(PredList, function(p) combinedAUC(p, Y))))
  
  while(sum.weights < iter) { 
    sum.weights   <- sum.weights + 1L
    pred          <- lapply(1:length(PredList), function(i) (PredList[[i]] + pred) * 1L/sum.weights)
    errors        <- unlist(lapply(pred, function(p) combinedAUC(p, Y)))
    best          <- which.max(errors)
    weights[best] <- weights[best] + 1L
    pred          <- pred[[best]] * sum.weights
    maxtest       <- max(errors)
