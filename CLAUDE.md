...
The former `hrfdecoder` integration is archived rather than part of the active
package surface.
...
- Archived vignette and notes: `archive/hrfdecoder/vignettes/` and `archive/hrfdecoder/docs/`

The entire `archive/` tree is excluded from package builds via
`.Rbuildignore`, so these materials remain available in the repository without
bringing `hrfdecoder` back into the shipped package or active pkgdown site.
