...
Your searchlight has 200 voxels but only 40 trials -- far more features than observations. Without feature selection, classifiers may overfit to noise, producing inflated accuracy inside the training set that fails to generalize. Feature selection identifies the most discriminative voxels _within each cross-validation fold_, keeping only those that carry genuine signal.
...
```{r ftest_top_k, message=FALSE, warning=FALSE}
...
```{r ftest_top_p, message=FALSE, warning=FALSE}
...
```{r apply_ftest_top_k, message=FALSE, warning=FALSE}
...
```{r apply_ftest_top_p, message=FALSE, warning=FALSE}
...
```{r catscore_method, message=FALSE, warning=FALSE}
