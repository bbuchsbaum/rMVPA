...
vignette: |
  %\VignetteIndexEntry{REMAP‑RRR — Domain‑Adaptive Cross‑Decoding}
...
The chunks in this vignette are shown but not evaluated during package builds
because reduced-rank adapters can be slow on some systems; they are complete
commands you can run interactively.

```{r minimal_example, eval=FALSE}
set.seed(1)
...
```{r inspect_diagnostics, eval=FALSE}
...
```{r searchlight_usage, eval=FALSE}
