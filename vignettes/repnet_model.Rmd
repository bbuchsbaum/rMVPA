...
> *ReNA-RC* is one of four cross-domain representational models in rMVPA. For the section's terminology and how to choose between *ReNA-RC*, *ReNA-Map*, *ReNA-RM*, and *REMAP-RRR*, see the glossary at the top of `vignette("Naive_Cross_Decoding")`.
...
A seed RDM is a `K x K` matrix of dissimilarities between items. ReNA-RC asks how strongly each ROI's representational geometry resembles this seed (optionally controlling for confounds). The toy example below uses a six-item ordinal seed: items closer in index are predicted to be more similar.
...
```{r repnet-seed-plot, echo = FALSE, fig.cap = "Left: seed RDM (an ordinal stack — small dissimilarity near the diagonal). Right: confound 'adjacent' RDM (1 for index-adjacent pairs, 0 otherwise). ReNA-RC measures the seed's variance unique to each ROI's geometry, after partialling out the confound.", fig.width = 6.5, fig.height = 3.4, out.width = "85%"}
op <- par(mfrow = c(1, 2), mar = c(2.5, 2.5, 1.6, 0.5))
image(seq_len(K), seq_len(K), S,
      col = hcl.colors(48, "YlOrBr", rev = TRUE),
      axes = FALSE, xlab = "", ylab = "", main = "seed RDM")
axis(1, at = seq_len(K), labels = items, las = 1)
axis(2, at = seq_len(K), labels = items, las = 1); box()
image(seq_len(K), seq_len(K), Adj,
      col = c("white", "#3a5b97"),
      axes = FALSE, xlab = "", ylab = "", main = "adjacent (confound)")
axis(1, at = seq_len(K), labels = items, las = 1)
axis(2, at = seq_len(K), labels = items, las = 1); box()
par(op)
```
...
```{r repnet_quick_start, eval=FALSE}
library(rMVPA)
...
```{r repnet_session_info, eval=FALSE}
sessionInfo()
```
