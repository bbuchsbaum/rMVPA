...
> *ReNA-Map* is one of four cross-domain representational models in rMVPA. For the section's terminology and how to choose between *ReNA-RC*, *ReNA-Map*, *ReNA-RM*, and *REMAP-RRR*, see the glossary at the top of `vignette("Naive_Cross_Decoding")`.
...
```{r repmap-seed-plot, echo = FALSE, fig.cap = "Left: seed feature matrix (rows = items, columns = features). Right: implied item-by-item similarity (centered features dot product). ReNA-Map asks each ROI how many of these feature dimensions it can recover.", fig.width = 7, fig.height = 3.4, out.width = "85%"}
op <- par(mfrow = c(1, 2), mar = c(3, 3, 1.6, 0.5))
image(seq_len(P), seq_len(K), t(seedF),
      col = hcl.colors(48, "Blue-Red 3"),
      axes = FALSE, xlab = "feature", ylab = "item", main = "seed features")
axis(1, at = seq_len(P)); axis(2, at = seq_len(K), labels = items, las = 1); box()
image(seq_len(K), seq_len(K), seed_sim,
      col = hcl.colors(48, "Blue-Red 3"),
      axes = FALSE, xlab = "", ylab = "", main = "implied item similarity")
axis(1, at = seq_len(K), labels = items, las = 1)
axis(2, at = seq_len(K), labels = items, las = 1); box()
par(op)
```
...
```{r repmap_quick_start, eval=FALSE}
library(rMVPA)
...
```{r repmap_session_info, eval=FALSE}
sessionInfo()
```
