...
# Safe histogram helper that avoids base title() path
safe_plot_hist <- function(x, title = "Histogram") {
  x <- as.numeric(x)
  x <- x[is.finite(x)]
  if (!length(x)) {
  ...
  bw <- if (length(h$breaks) > 1) min(diff(h$breaks)) else 1
  y_max <- max(h$counts, 1)
  # Draw using grid to avoid base graphics device issues under callr
  grid::grid.newpage()
  grid::pushViewport(grid::plotViewport(margins = c(4, 4, 2, 2)))
  grid::pushViewport(grid::dataViewport(xscale = range(h$breaks, finite = TRUE), yscale = c(0, y_max)))
  # bars
  for (i in seq_along(h$mids)) {
    grid::grid.rect(x = h$mids[i], y = 0, width = bw, height = h$counts[i],
                    just = c("center", "bottom"),
                    default.units = "native",
                    gp = grid::gpar(fill = "grey80", col = NA))
  }
  # axes
  grid::grid.xaxis()
  grid::grid.yaxis()
  # labels
  grid::grid.text(title, y = grid::unit(1, "npc") - grid::unit(1.5, "lines"))
  grid::grid.text("AUC", y = grid::unit(-2, "lines"))
  grid::upViewport(2)
  invisible(NULL)
}

# Toggle plotting in vignettes (disable by default for pkgdown stability)
plot_ok <- identical(Sys.getenv("RMVPA_VIGNETTE_PLOTS"), "true")
```
...
if (plot_ok && !is.null(result$results$AUC)) {
  ...
  auc_vals <- auc_vals[is.finite(auc_vals)]
  if (length(auc_vals) > 0) {
    safe_plot_hist(auc_vals, title = "Randomized Searchlight AUC (centered)")
  } else {
    message("No finite AUC values to plot.")
  }
}
