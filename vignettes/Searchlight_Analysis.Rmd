...
# Safe histogram helper to avoid base-graphics segfaults in pkgdown/callr
safe_plot_hist <- function(x, title = "Histogram") {
  ...
  if (in_pkgdown && requireNamespace("ggplot2", quietly = TRUE)) {
    df <- data.frame(x = x)
    p <- ggplot2::ggplot(df, ggplot2::aes(x = x)) +
      ggplot2::geom_histogram(bins = 30, fill = "grey80", color = "white") +
      ggplot2::labs(title = title, x = "AUC", y = "Count") +
      ggplot2::theme_minimal()
    print(p)
  } else if (in_pkgdown) {
    # Create a minimal placeholder plot so pkgdown/knitr still emit a figure
    # Avoid title()/labels to steer clear of device issues
    grDevices::plot.new()
    graphics::rect(0, 0, 1, 1, col = "white", border = NA)
    graphics::text(0.5, 0.5, labels = title, cex = 0.9)
    invisible(NULL)
  } else if (requireNamespace("ggplot2", quietly = TRUE)) {
    # Prefer ggplot2 when available to avoid device quirks
    df <- data.frame(x = x)
    p <- ggplot2::ggplot(df, ggplot2::aes(x = x)) +
      ggplot2::geom_histogram(bins = 30, fill = "grey80", color = "white") +
      ggplot2::labs(title = title, x = "AUC", y = "Count") +
    ...
    # Fallback to base only outside pkgdown
    hist(x, main = title, xlab = "AUC", col = "grey80", border = "white")
  }
}
```
...
if (!is.null(result$results$AUC)) {
  safe_plot_hist(neuroim2::values(result$results$AUC), title = "Searchlight AUC (centered)")
}
...
if (!is.null(result$results$AUC)) {
  safe_plot_hist(neuroim2::values(result$results$AUC), title = "Randomized Searchlight AUC (centered)")
}
