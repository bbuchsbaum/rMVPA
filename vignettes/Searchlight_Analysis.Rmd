...
if (!is.null(result$results$AUC)) {
  ...
  if (length(auc_vals) > 0) {
    hist(auc_vals, main = "Searchlight AUC (centered)", xlab = "AUC", col = "grey80", border = "white")
  } else {
...
if (!is.null(result$results$AUC)) {
  ...
  auc_vals <- auc_vals[is.finite(auc_vals)]
  if (length(auc_vals) > 0) {
    hist(auc_vals, main = "Randomized Searchlight AUC (centered)", xlab = "AUC", col = "grey80", border = "white")
  } else {
    message("No finite AUC values to plot.")
  }
}
...
model <- mvpa_model(model=rf_model, dataset=dataset$dataset, design=dataset$design, crossval=crossval)
