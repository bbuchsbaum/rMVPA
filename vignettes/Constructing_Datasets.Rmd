...
The dataset bundles a 4-D `NeuroVec` (voxels × time) and a binary `NeuroVol` mask. Two pictures make the shape concrete: a mid-axial slice of the mask, and the trial × run grid of the design.

```{r synthetic-visual, echo = FALSE, fig.cap = "Left: a mid-axial slice of the binary mask (active voxels in dark blue). Right: each trial coloured by run; runs split into equal-length blocks of trials.", fig.width = 7, fig.height = 3.4}
op <- par(mfrow = c(1, 2), mar = c(2.5, 2.5, 1.5, 0.5))
mask_arr <- as.array(ds$dataset$mask)
mid_z <- ceiling(dim(mask_arr)[3] / 2)
image(mask_arr[, , mid_z], col = c("white", "#3a5b97"),
      axes = FALSE, main = "mask slice")
box()
