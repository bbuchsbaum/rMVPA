...
`lambda_objective = "mse"` is the default and is the only objective available
for GCV and analytic LOO. If identification is the primary estimand, use
blocked tuning with `lambda_objective = "pattern_rank_percentile"`. Every
candidate rank is then computed wholly inside one held-out inner block. Rank
tuning uses the empirical optimum by default; set `lambda_one_se = TRUE` only
if the stronger-regularisation tie-break is part of the planned analysis.
