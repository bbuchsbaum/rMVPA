...
typically a `classification_result` – and must return a **numeric vector**
of scalar metrics (named if you return more than one):
...
You pass this function to `mvpa_model()` via the `performance` argument:
...
Each named entry from `my_perf()` (`accuracy`, `mean_pobs`, `max_pobs`) becomes
one volumetric map in the searchlight or regional result.

## Using split_by with custom performance

If your design specifies `split_by`, `mvpa_design` stores the grouping in
`split_groups`, and `mvpa_model()` automatically applies your metric both
globally and within each group:
...
Internally, `rMVPA` computes `my_perf(result)` on the full result and then
again for each split group in `mvdes$split_groups`. Metrics for group `run=1`
are suffixed with `"_run1"`, those for `run=2` with `"_run2"`, and so on. When
a test set is present, the grouping indices are interpreted in the design
space and mapped to the appropriate rows of the result via `testind`.

## Summary

The `run_custom_regional` and `run_custom_searchlight` functions provide a
powerful mechanism to extend `rMVPA`'s capabilities at the ROI/searchlight
level, while the `performance` hook in `mvpa_model()` lets you swap in
custom scalar metrics for standard MVPA workflows. Together, these tools
allow you to integrate bespoke analyses into `rMVPA` with minimal boilerplate.

For implementation details, see `run_custom_regional()`, `run_custom_searchlight()`, and `mvpa_model()`.
