...
![MS-ReVE in six panels. (1) contrast matrix C (K conditions x Q
contrasts). (2a/2b) each contrast's predicted dissimilarity Delta_q =
-c_q c_q^T. (3) a searchlight neural RDM. (4) beta_q from regressing the
neural RDM on the Delta_q stack. (5) signed voxel score beta_q \*
Delta\_{q,v}: positive voxels pull the contrast in its predicted
direction, negative voxels oppose. (6) summary equations. The figure
uses synthetic patterns separate from the example below; it is here to
anchor the symbols you will see in the metrics
section.](Contrast_RSA_files/figure-html/msreve-schematic-1.png)
...
direction, negative voxels oppose. (6) summary equations. The figure
uses synthetic patterns separate from the example below; it is here to
anchor the symbols you will see in the metrics section.
