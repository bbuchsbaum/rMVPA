...
Your searchlight has 200 voxels but only 40 trials – far more features
than observations. Without feature selection, classifiers may overfit to
noise, producing inflated accuracy inside the training set that fails to
generalize. Feature selection identifies the most discriminative voxels
*within each cross-validation fold*, keeping only those that carry
genuine signal.

Neuroimaging datasets often contain far more features than informative
signal. Feature selection reduces dimensionality, improves
...
`rMVPA`, you configure selection with a feature‑selector object. The
package supports an ANOVA‑based F‑test (“FTest”) and
correlation‑adjusted t‑scores (“catscore”, via `sda.ranking`) to rank
features, together with simple cutoffs: keep the top `k` features or the
top proportion `p` (e.g., `p = 0.1` keeps the top 10%).
...
of features using the top_p option. In the example below, we select the
...
Now, let’s use the top_p option. This will select a proportion of the
...
Alternatively, you can use the `catscore` method to perform feature
selection. The `catscore` method computes a correlation-adjusted t-score
...
`rMVPA`, the integration of feature selection into cross-validation
workflows helps ensure that models are built on unbiased, relevant
subsets of data. You can choose between different methods (FTest or
catscore) and cutoff strategies (top_k vs top_p) based on your specific
analysis needs.
